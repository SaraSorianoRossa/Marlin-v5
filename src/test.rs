...
mod marlin {
    ...
    fn test_circuit(num_constraints: usize, num_variables: usize) {
        ...
        for _ in 0..100 {
            ...
            let (index_pk, index_vk, matrix_a, matrix_b, matrix_c) = MarlinInst::index(&universal_srs, circ.clone()).unwrap();
            ...
            for _ in 0..2{                
                let (proof, alpha, eta_a, eta_b, eta_c, beta, t_beta, num_constraints_i, num_input_variables_i) = MarlinInst::prove(&index_pk, circ, rng).unwrap();
                println!("Called prover");

                assert!(MarlinInst::verify(&index_vk, &[c, d], &proof, rng, &t_beta).unwrap());
                println!("Called verifier");
                println!("\nShould not verify (i.e. verifier messages should print below):");
                assert!(!MarlinInst::verify(&index_vk, &[a, a], &proof, rng, &t_beta).unwrap());

                let variables = vec![alpha, eta_a, eta_b, eta_c, beta, t_beta];
    ...
    fn prove_and_test_outlining() {
        ...
        let matrices = vec![matrix_a.clone(), matrix_b.clone(), matrix_c.clone()];
        for _ in 0..2{                
            let (proof, alpha, eta_a, eta_b, eta_c, beta, t_beta, num_constraints_i, num_input_variables_i) = MarlinInst::prove(&index_pk, circ.clone(), rng).unwrap();
            println!("Called prover");
            ...
            assert!(MarlinInst::verify(&index_vk, &inputs, &proof, rng, &t_beta).unwrap());
            println!("Called verifier");
